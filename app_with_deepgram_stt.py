...
def initialize_models():
    ...
    """
    global sambanova_client, deepgram_client, tts_pipeline
    ...
    # Initialize Deepgram client for STT
    print("Setting up Deepgram client for STT...")
...
def deepgram_stt(audio_data: NDArray, sample_rate: int = 16000) -> str:
    """
    ...
    try:
        ...
        noise_threshold = 0.08  # This is a relative value
        ...
        try:
            # Create options using the PrerecordedOptions class
            ...
            # Read the file and create a source dictionary
            with open(temp_path, "rb") as audio_file:
                file_bytes = audio_file.read()
                
            # Create a source dictionary with the buffer
            source = {"buffer": file_bytes}
            
            # Call the transcribe_file method directly
            response = deepgram_client.listen.prerecorded.v("1").transcribe_file(source, options)
            
            # Parse the response
            if response and hasattr(response, "results"):
                # Try to access the transcript from the response
                try:
                    transcript = response.results.channels[0].alternatives[0].transcript
                    return transcript
                except (AttributeError, IndexError) as e:
                    print(f"Failed to extract transcript from response object: {e}")
                
                # Fallback to JSON parsing if object access fails
                try:
                    ...
                    if (response_dict and 
                        ...
                        alternatives = response_dict["results"]["channels"][0]["alternatives"]
            ...
            print(f"Deepgram API call error: {e}")
            return ""
...
def response(
    ...
    if sambanova_client is None or deepgram_client is None or tts_pipeline is None:
        initialize_models()
    ...
    # Use Deepgram for transcription with audio
    start = time.time()
    text = deepgram_stt(audio_data, sample_rate)
...
# Add a property to store the startup data
on_stream_startup.data = {}
    ...
    stream = Stream(
        ...
        handler=ReplyOnPause(
            ...
            algo_options=AlgoOptions(
                audio_chunk_duration=1.0,      # Increased from 0.8 to 1.0 for better speech detection
                started_talking_threshold=0.9,  # Increased from 0.8 to 0.9 to require clearer speech onset
                speech_threshold=0.85,          # Increased from 0.8 to 0.85 to filter out low-level sounds
            ),
        ),
